...
OUTPUT_FILE = "docs/Report_Content_Draft.md"
...
The intersection control problem is strictly defined as a Decentralized Partially Observable Markov Decision Process (Dec-POMDP) represented by the tuple $\langle \mathcal{N}, \mathcal{S}, \mathcal{A}, \mathcal{P}, \mathcal{R}, \Omega, \mathcal{O}, \gamma \rangle$.
...
The state transition $\mathcal{S} \times \mathbf{A} \rightarrow \mathcal{P}(\mathcal{S}')$ is deterministically governed by the macroscopic kinematics simulated within the SUMO engine, largely adhering to the Krauss Car-Following Model and the LC2013 lane-changing equations.
...
$$ r_i^{(t)} = - \alpha \sum_{l \in L_i} queue(l)^{(t)} - \beta \sum_{l \in L_i} wait(l)^{(t)} - \lambda \cdot \text{pressure}_i^{(t)} $$
...
1. **Avg Waiting Time:** MAPPO effectively halved the global average waiting time compared to fixed-time metrics (descending from $\approx 68.7s$ to an exceptional $\approx 31.4s$). Relative to the NSTLight model ($\approx 44.2s$), our model secured an aggressive 28% efficiency boost.
